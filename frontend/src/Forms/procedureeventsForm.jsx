  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/api/procedureevents",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify(procedureEventData),
        }
      );

      if (response.ok) {
        console.log("ProcedureEvents form submitted successfully!");
        // Handle success (e.g., navigation or other actions)
      } else {
        console.error("Failed to submit ProcedureEvents form:", response);
      }
    } catch (error) {
      console.error("Error submitting ProcedureEvents form:", error);
    }
  };
...
export default ProcedureEventsForm;
